...
-   Going to back the general form of Hooke’s Law (`$\sigma_{ij} = C_{ijkl}\epsilon_{kl}$`), this gives the symmetry condition
...
-   We know that `$\sigma_{ij}n_j = 0$` on surfaces where tractions are defined and that `$u_i = 0$` on the other surfaces, so the first integral goes to zero
-   We also know by equilibrium that `$\sigma_{ij,j}$`, so the second integral will also be 0
